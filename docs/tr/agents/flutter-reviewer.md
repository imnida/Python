...
- Bug'lar, güvenlik, veri kaybı ve doğruluk üzerine stil yerine önceliklendirin
...
- **Pure-Dart katmanlarına sızan framework** — Eğer proje framework-free olması amaclanan bir domain/model katmanına sahipse, Flutter veya platform kodu import etmemelidir
...
- **Business logic'te doğrudan instantiation** — State manager'lar dependency'leri injection ile almalıdır, internal olarak construct etmemelidirler
...
- **Eksik error/loading state'leri** — Her async işlem loading, success ve error'ı ayrı ayrı modellemelidir
...
- Diğer çözümlerde, inter-component iletişimi için belgelenen konvansiyonları takip edin
...
- **Eksik `RepaintBoundary`** — Bağımsız yeniden boyanan karmaşık subtree'ler sarmalanmalıdır
...
- **`late` aşırı kullanımı** — Nullable tipler veya constructor initialization'ı tercih edin
...
- **UI'ye ulaşan ham exception'lar** — Presentation katmanından önce kullanıcı dostu, yerelletirilmiş mesajlara map edin
