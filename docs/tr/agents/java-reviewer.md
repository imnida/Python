...
- **PII/token logging**: Şifreler veya token'ları açığa çıkaran auth kodu yakınında `log.info(...)` çağrıları
...
- **Kaçırılmış pattern matching**: Açık cast ile takip edilen `instanceof` kontrolü — pattern matching kullanın (Java 16+)
