...
Enhanced prompt'u bekle, **orijinal $ARGUMENTS'ı tüm sonraki fazlar için enhanced sonucuçla değiştir**.
...
- Requirement'larda hala belirsizlik varsa, kullanıcı için yönlendirici sorular **MUTLAKA** çıktla
...
Claude'un sentezlenmiş planındaki eksiklik riskini azaltmak için, her iki modelin de "plan taslakları" çıktlamasını parallel yaptır (yine **dosya değiştirmesine izin verilmez**):
...
3. **Kalın metinle** prompt çıktla (MUTLAKA gerçek kaydedilen dosya yolunu kullan):
...
**KESINLIKLE YASAK**:
- Kullanıcıya "Y/N" sor sonra otomatik çalıştır (execution `/ccg:execute`'un sorumluluuğu)
