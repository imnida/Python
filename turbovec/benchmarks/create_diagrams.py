...
def insert_panels(arch):
    panels = {"st": [], "mt": []}
    for dim in (1536, 3072):
        for bw in (2, 4):
            for th in ("st", "mt"):
                entry = load_json(f"speed_insert_d{dim}_{bw}bit_{arch}_{th}.json")
                panels[th].append(
                    {
                        "label": f"d={dim}|{bw}-bit",
                        "tq": entry["tq_vecs_per_sec"],
                        "faiss": entry["faiss_vecs_per_sec"],
                    }
                )
    return panels
...
def write_insert_panel(arch, hw_label, thread_key, thread_label, filename):
    panels = insert_panels(arch)
    width, height = 900, 460
    margin = {"top": 82, "right": 32, "bottom": 108, "left": 100}
    pw = width - margin["left"] - margin["right"]
    ph = height - margin["top"] - margin["bottom"]
    px = margin["left"]
    py = margin["top"]
    ...
    parts = [
        paired_panel(px, py, pw, ph, thread_label, panels[thread_key],
                     tick_fmt=k_fmt, value_fmt=k_fmt, y_max=y_max),
        f'<text x="26" y="{py + ph/2}" transform="rotate(-90, 26, {py + ph/2})" class="axis">vecs / sec</text>',
        legend_tq_faiss(margin["left"], height - 26),
    ]
    body = "\n".join(parts)

    svg = f"""<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" role="img" aria-label="Insert Throughput — {xe(hw_label)} — {xe(thread_label)}">
  {style_block()}
  <rect width="100%" height="100%" fill="#ffffff" />
  <text x="{margin["left"]}" y="32" class="title">Insert Throughput — {xe(hw_label)} — {xe(thread_label)}</text>
  <text x="{margin["left"]}" y="52" class="subtitle">100K vectors, median of 5 runs, FAISS trained on first 50K</text>
  {body}
</svg>
"""
    out = os.path.join(DOCS_DIR, filename)
...
def remove_panels(arch):
    panels = {"st": [], "mt": []}
    for dim in (1536, 3072):
        for bw in (2, 4):
            for th in ("st", "mt"):
                entry = load_json(f"speed_remove_d{dim}_{bw}bit_{arch}_{th}.json")
                panels[th].append(
                    {
                        "label": f"d={dim}|{bw}-bit",
                        "tq": entry["id_map_vecs_per_sec"],
                        "faiss": entry["swap_remove_vecs_per_sec"],
                    }
                )
    return panels


def legend_idmap_swap(x, y):
    parts = [
        f'<rect x="{x}" y="{y - 10}" width="14" height="14" rx="3" fill="{C["tq"]}" stroke="{C["tq_stroke"]}" stroke-width="1.5" />',
        f'<text x="{x + 22}" y="{y + 1}" class="legend" style="fill: {C["tq_text"]};">IdMapIndex.remove()</text>',
        f'<rect x="{x + 210}" y="{y - 10}" width="14" height="14" rx="3" fill="{C["faiss"]}" />',
        f'<text x="{x + 232}" y="{y + 1}" class="legend">swap_remove()</text>',
    ]
    return "\n".join(parts)


def write_remove_panel(arch, hw_label, thread_key, thread_label, filename):
    panels = remove_panels(arch)
    width, height = 900, 460
    margin = {"top": 82, "right": 32, "bottom": 108, "left": 100}
    pw = width - margin["left"] - margin["right"]
    ph = height - margin["top"] - margin["bottom"]
    ...
    parts = [
        paired_panel(px, py, pw, ph, thread_label, panels[thread_key],
                     tick_fmt=k_fmt, value_fmt=k_fmt, y_max=y_max),
        f'<text x="26" y="{py + ph/2}" transform="rotate(-90, 26, {py + ph/2})" class="axis">vecs / sec</text>',
        legend_idmap_swap(margin["left"], height - 26),
    ]
    body = "\n".join(parts)

    svg = f"""<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" role="img" aria-label="Remove Throughput — {xe(hw_label)} — {xe(thread_label)}">
  {style_block()}
  <rect width="100%" height="100%" fill="#ffffff" />
  <text x="{margin["left"]}" y="32" class="title">Remove Throughput — {xe(hw_label)} — {xe(thread_label)}</text>
  <text x="{margin["left"]}" y="52" class="subtitle">50K vectors, drain all via O(1) swap-remove, median of 5 runs</text>
  {body}
</svg>
"""
    out = os.path.join(DOCS_DIR, filename)
...
if __name__ == "__main__":
    ...
    write_remove_panel("arm", "ARM (Apple M3 Max)", "mt", "Multi-threaded",
                       filename="arm_remove_mt.svg")
    write_remove_panel("x86", "x86 (Intel Sapphire Rapids, 8 vCPUs)", "st", "Single-threaded",
                       filename="x86_remove_st.svg")
    write_remove_panel("x86", "x86 (Intel Sapphire Rapids, 8 vCPUs)", "mt", "Multi-threaded",
                       filename="x86_remove_mt.svg")
